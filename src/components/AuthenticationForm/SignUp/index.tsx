...
export default function SignIn({ setisSignUp, isSignUp }: SignInProps) {
    ...
    const schema = z.object({
        ...
        confirmPassword: z.string().min(8, { message:  t("login-form.password-error") })
    ...
    const { register, handleSubmit, formState: { errors } } = useForm<SigUpFormData>({
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="grid text-main-brown">
            ...
            <SwitchForm setisSignUp={setisSignUp} isSignUp={isSignUp} buttonLabel={t("login-form.login")} description={t("login-form.description-signup")}/>
        </form>
    );
}
